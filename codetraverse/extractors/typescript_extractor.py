...
class TypeScriptComponentExtractor(ComponentExtractor):
    def __init__(self):
        ...
        if not self.root_folder:
            raise RuntimeError(
                "ROOT_DIR environment variable must be set for TypeScript extractor."
            )
    ...
    def resolve_imports(self, node, code, current_file_path, root_folder):
        ...
        if node.type == "import_statement":
            ...
            if "from" in import_text:
                ...
                abs_source = os.path.normpath(
                    os.path.join(
                        os.path.dirname(current_file_path), source_path + ".ts"
                    )
                )
        ...
        for child in node.children:
            imports.update(
                self.resolve_imports(child, code, current_file_path, root_folder)
            )
    ...
    def collect_imports_for_file(self, root_node, code, current_file, root_folder):
        ...
        for child in root_node.children:
            if child.type == "import_statement":
                imports.update(
                    self.resolve_imports(child, code, current_file, root_folder)
                )
        return imports
    ...
    def process_file(self, file_path):
        ...
        imports = self.collect_imports_for_file(
            tree.root_node, code, file_path, self.root_folder
        )
    ...
    def walk_node(
        self, node, code, file_path, root_folder, rel_module_path, imports, context
    ):
        results = []
        ...
        if node.type in ("function_declaration", "function_signature"):
            function_calls = self.extract_function_calls(
                node, code, rel_module_path, imports
            )
        ...
        if node.type in ("class_declaration", "abstract_class_declaration", "class"):
            ...
            if not class_name:
                return results
            function_calls = self.extract_function_calls(
                node, code, rel_module_path, imports, class_name
            )
                ...
                if child.type == "class_body":
                    for member in child.children:
                        ...
                        if member.type == "method_definition":
                            ...
                            if method_name:
                                method_path = (
                                    f"{rel_module_path}::{class_name}::{method_name}"
                                )
    ...
    def extract_function_calls(
        self, node, code, rel_module_path, imports, class_name=None
    ):
        calls = []
