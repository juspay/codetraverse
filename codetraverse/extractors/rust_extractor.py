...
class RustComponentExtractor(ComponentExtractor):
    ...
    def _extract_import_mappings(self, node: Node, src: bytes, base_path: str):
        if node.type == "identifier":
            ...
            if path_node and name_node:
                ...
                full_path = (
                    f"{base_path}::{path_str}::{name_str}"
                    if base_path
                    else f"{path_str}::{name_str}"
                )
    ...
    def _extract_type_info(self, node: Node, src: bytes) -> str:
        ...
        if node.type == "generic_type":
            ...
            base_text = (
                src[base_type.start_byte : base_type.end_byte].decode("utf8")
                if base_type
                else ""
            )
            args_text = (
                src[type_args.start_byte : type_args.end_byte].decode("utf8")
                if type_args
                else ""
            )
        ...
        if parent:
            for sibling in parent.children:
                if (
                    ...
                    attr_text = src[sibling.start_byte : sibling.end_byte].decode(
                        "utf8"
                    )
    ...
    def _process_node(
        self, node: Node, src: bytes, file_path: str, current_module_path: str = None
    ) -> dict:
        if current_module_path is None:
            current_module_path = (
                self.module_stack[-1] if self.module_stack else self.current_module_path
            )
        primary_kinds = {
            "mod_item",
            "use_declaration",
        ...
        if node.type == "mod_item":
            full_module_path = (
                f"{current_module_path}::{name}"
                if current_module_path != "crate"
                else f"crate::{name}"
            )
        else:
        ...
        comp = {
            ...
            "span": span,
            "code": src[node.start_byte : node.end_byte].decode(
                "utf8", errors="ignore"
            ),
    ...
    def _extract_function_info(self, node: Node, src: bytes, comp: dict):
        ...
        if params_node:
            for param in params_node.named_children:
                if param.type == "parameter":
                    ...
                    param_info = {
                        "name": (
                            src[pattern.start_byte : pattern.end_byte].decode("utf8")
                            if pattern
                            else ""
                        ),
                        "type": (
                            self._extract_type_info(type_node, src)
                            if type_node
                            else None
                        ),
                    }
        ...
        if type_params:
            comp["type_parameters"] = src[
                type_params.start_byte : type_params.end_byte
            ].decode("utf8")
        where_node = None
        for child in node.children:
                                ...
                                variant_info["fields"].append(
                                    {
                                        ...
                                        "type": (
                                            self._extract_type_info(field_type, src)
                                            if field_type
                                            else ""
                                        ),
                                    }
    ...
    def _extract_use_info(self, node: Node, src: bytes, comp: dict):
        def extract_imports_from_node(n: Node, base_path: str = ""):
            if n.type == "identifier":
                import_path = (
                    f"{base_path}::{src[n.start_byte:n.end_byte].decode('utf8')}"
                    if base_path
                    else src[n.start_byte : n.end_byte].decode("utf8")
                )
                ...
                if path_node and name_node:
                    path_str = src[path_node.start_byte : path_node.end_byte].decode(
                        "utf8"
                    )
                    name_str = src[name_node.start_byte : name_node.end_byte].decode(
                        "utf8"
                    )
                    full_path = (
                        f"{base_path}::{path_str}::{name_str}"
    ...
    def _traverse_and_extract(self, node: Node, src: bytes, comp: dict, file_path: str):
        def walk(n: Node):
            ...
            if t == "call_expression":
                ...
                if macro_node:
                    macro_text = src[
                        macro_node.start_byte : macro_node.end_byte
                    ].decode("utf8")
                ...
                if pat:
                    var_info = {
                        ...
                        "value": (
                            src[init.start_byte : init.end_byte].decode("utf8")
                            if init
                            else None
                        ),
                        "span": {
                ...
                type_info = {"type": type_text, "resolved_type": resolved_type}
                if not any(
                    existing["type"] == type_text for existing in comp["types_used"]
                ):
