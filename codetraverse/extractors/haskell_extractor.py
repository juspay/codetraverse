...
class HaskellComponentExtractor(ComponentExtractor):
    ...
    def process_file(self, file_path):
                ...
                if module_node:
                    for module_id in module_node.children:
                        if module_id.type == "module_id":
                            module_path.append(
                                src[module_id.start_byte : module_id.end_byte].decode()
                            )
                self.current_module = ".".join(module_path)
                break

        raw_groups = [
            self.extract_top_level_components(i, src, import_map=self.import_map)
        ...
        for comp in self.all_components:
            if comp["kind"] == "function":
                comp["type_dependencies"] = self.find_type_dependencies(
                    comp["name"], self.all_components
                )
    ...
    def extract_top_level_components(self, root_node, src_bytes, import_map):
            ...
            if mod_n:
                for mid in mod_n.named_children:
                    if mid.type == "module_id":
                        module_path.append(
                            src_bytes[mid.start_byte : mid.end_byte].decode()
                        )
            ...
            if exp_n:
                for item in exp_n.named_children:
                    if item.type == "module_export":
                        ...
                        if alias:
                            exports.append(
                                src_bytes[alias.start_byte : alias.end_byte].decode()
                            )
                    elif item.type in ("export", "import_name", "name"):
                        txt = (
                            src_bytes[item.start_byte : item.end_byte].decode().strip()
                        )
                        exports.append(txt)
                ...
                for sib in parent.children:
                    for child in sib.children:
                        ...
                        if child.type in (
                            ...
                            name_n = child.child_by_field_name(
                                "name"
                            ) or child.child_by_field_name("variable")
                            if name_n:
                                exports.append(
                                    src_bytes[
                                        name_n.start_byte : name_n.end_byte
                                    ].decode()
                                )
        ...
        for child in root_node.children:
            if child.type == "header":
                ...
                if module_node:
                    for module_id in module_node.children:
                        if module_id.type == "module_id":
                            module_path.append(
                                src_bytes[
                                    module_id.start_byte : module_id.end_byte
                                ].decode()
                            )
                components.append(
                    {
                ...
                body_code = (
                    src_bytes[body_node.start_byte : body_node.end_byte].decode()
                    if body_node
                    else ""
                )
                ...
                comp["function_calls"] = self.extract_function_calls(
                    body_code, import_map, self.current_module
                )
                ...
                components.append(comp)
                comp["reexported_from"] = reexported_modules.get(
                    self.current_module, []
                )
            elif child.type == "instance":
                instance_comp = self.extract_instance_component(
                    child, src_bytes, import_map
                )
                if instance_comp:
                ...
                data_comp = self.extract_data_type_component(
                    child, src_bytes, import_map
                )
                if data_comp:
        ...
        for node in function_node.children:
            if node.type == "local_binds":
                for bind_node in node.children:
                    ...
                    where_defs.append({"kind": "function", "name": name, "code": code})
        return where_defs
    ...
    def extract_import_component(self, import_node, src_bytes):
        ...
        module_name = (
            src_bytes[module_node.start_byte : module_node.end_byte].decode()
            if module_node
            else None
        )
                ...
                if name_child.type == "import_name":
                    for id_child in name_child.children:
                        if id_child.type in ["name", "variable"]:
                            import_list.append(
                                src_bytes[
                                    id_child.start_byte : id_child.end_byte
                                ].decode()
                            )
    ...
    def extract_single_constructor(self, constructor_node, src_bytes):
        constructor_info = {"type": "constructor", "name": "Unknown", "fields": []}
        for child in constructor_node.children:
            if child.type == "record":
                constructor_info["type"] = "record"
                constructor_info["name"] = self.extract_constructor_name(
                    child, src_bytes
                )
                constructor_info["fields"] = self.extract_record_fields(
                    child, src_bytes
                )
            elif child.type == "constructor":
                constructor_info["name"] = src_bytes[
    ...
    def extract_field_info(self, field_node, src_bytes):
        ...
        type_txt = (
            src_bytes[type_node.start_byte : type_node.end_byte].decode()
            if type_node
            else None
        )
        core = type_txt
        ...
        return {"name": field_name, "type": type_txt, "type_info": type_info}
    ...
    def _extract_qualified_type(self, qualified_node, src_bytes):
        ...
        base_node = qualified_node.child_by_field_name(
            "id"
        ) or qualified_node.child_by_field_name("name")
        base = (
            src_bytes[base_node.start_byte : base_node.end_byte].decode()
            if base_node
            else ""
        )
        ...
        if first and first in self.import_map:
            modules = [
                f"{imp}.{'.'.join(module_bits[1:])}" for imp in self.import_map[first]
            ]
        else:
                    ...
                    if decl.type == "declaration":
                        for inner_decl in decl.children:
                            if inner_decl.type == "bind":
                                method = self.extract_instance_method(
                                    inner_decl, src_bytes, import_map
                                )
                                if method:
                                    instance_methods.append(method)
                            elif inner_decl.type == "type_instance":
                                type_inst = self.extract_type_instance(
                                    inner_decl, src_bytes
                                )
                                if type_inst:
    ...
    def extract_qualified_info(self, qualified_node, src_bytes):
        ...
        if module_part and id_part:
            ...
            return {"name": id_part, "type": "simple", "context": "type_pattern"}
        else:
            fallback_name = src_bytes[
                qualified_node.start_byte : qualified_node.end_byte
            ].decode()
            return {
                "name": fallback_name,
                "type": "fallback",
    ...
    def extract_function_calls(
        self, func_code: str, import_map: dict, current_module: str
    ):
        ...
        for line in lines:
            ...
            if (
                "::" in line
                or line.strip().startswith("instance")
            ...
            for op in operators:
                if op in skip_keywords:
                    continue
                identifiers.append(
                    {"name": op, "type": "operator", "context": "operation"}
                )
            ...
            for record_match in record_pattern.finditer(line):
                ...
                identifiers.append(
                    {"name": record_match.group(0), "type": "record", "fields": fields}
                )

            if lambda_pattern.search(line):
                identifiers.append(
                    {"name": "λ", "type": "lambda", "context": "anonymous_function"}
                )
            ...
            for ctor in re.findall(r"\b([A-Z][a-zA-Z0-9_\']*)\b", line):
                if ctor in skip_keywords:
                    continue
                identifiers.append(
                    {"name": ctor, "type": "type_constructor", "context": "type_system"}
                )
            ...
            if "=" in line and "type" not in line:
                for var in re.findall(r"\b([a-z][a-zA-Z0-9_\']*)\b", line):
                    if var in skip_keywords:
                        continue
                    identifiers.append(
                        {"name": var, "type": "variable", "context": "binding"}
                    )
