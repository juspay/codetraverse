...
def extract_component_info(item: Dict) -> Dict[str, Any]:
    return {
        "name": item.get('name', 'Unknown'),
        "kind": item.get('kind', 'N/A')
    }

def getModuleInfo(fdep_folder: str, module_name: str) -> List[str]:
                ...
                if isinstance(data, list):
                    for item in data:
                        if isinstance(item, dict) and item.get("module") == module_name:
                            if 'name' in item:
                                exact_matches.append(extract_component_info(item))
                                # print(exact_matches)
    ...
    for file_path in json_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, list):
                    for item in data:
                        if isinstance(item, dict) and module_name in item.get("module", ""):
                            if 'name' in item:
                                lazy_matches.append(item['name'])
        except (json.JSONDecodeError, IOError):
            continue

    if lazy_matches:
        return sorted(list(set(lazy_matches)))
