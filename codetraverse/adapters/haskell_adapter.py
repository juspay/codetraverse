...
def adapt_haskell_components(raw_components, quiet: bool = True):
    ...
    for comp in raw_components:
        if comp["kind"] == "import" and comp.get("alias"):
            ...
            for node in nodes:
                if (
                    node["id"].startswith(f"{alias_id.split('::')[0]}::")
                    and node["category"] == "reexport"
                ):
                    edges.append(
                        {
                            "from": alias_id,
    ...
    for comp in tqdm(
        raw_components, total=len(raw_components), desc="Adapting Haskell components"
    ):
                    ...
                    if target_id not in node_ids:
                        ...
                        for node in nodes:
                            if node["id"] == target_id or (
                                node["category"] == "reexport"
                                and node["id"].endswith(f"::{base_name}")
                            ):
        ...
        if comp["kind"] == "data_type":
            for constructor in comp.get("constructors", []):
                for field in constructor.get("fields", []):
                    ...
                    target_id = (
                        f"{field_module}::{field_name}" if field_module else field_name
                    )
