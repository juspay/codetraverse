...
def adapt_python_components(raw_components, quiet=True):
    ...
    for comp in tqdm(
        raw_components, total=len(raw_components), desc="Adapting Python Components"
    ):
        if comp["kind"] == "function":
            fn_node = add_node(
                ...
                "function",
                {
                    ...
                    "location": {"start": comp["start_line"], "end": comp["end_line"]},
                },
            )

            for call in comp.get("function_calls", []):
                add_node(call, "function")
                edges.append({"from": comp["name"], "to": call, "relation": "calls"})
            ...
            for base in comp.get("base_classes", []):
                add_node(base, "class")
                edges.append({"from": comp["name"], "to": base, "relation": "inherits"})

            for method in comp.get("methods", []):
                method_node = add_node(
                    f"{comp['name']}::{method['name']}",
                    "method",
                    {
